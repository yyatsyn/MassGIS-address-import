...
if len(problem_pnt_addr3) > 0:
    problem_pnt_addr3.to_file(path + 'problem_pnt_addr_case_insensitive.shp')
...
if len(problem_bld_addr3) > 0:
    problem_bld_addr3.to_file(path + 'problem_bld_addr_case_insensitive.shp')
...
os.system("mkdir " + path + "cty_towns/problem_mgis_case_insensitive")
for cty_town in cty_towns:
    ...
    if len(problem_mgis3) > 0:
        problem_mgis3.to_file(path + "cty_towns/problem_mgis_case_insensitive/" + cty_town + '.shp')
...
for cty_town in cty_towns:
    print("Processing " + cty_town)
    try:
        cur = gpd.read_file(path + "cty_towns/problem_mgis_case_insensitive/" + cty_town + '.shp')
...
to_add.to_csv(path + "missing_streets_case_insensitive.csv", index=False)
